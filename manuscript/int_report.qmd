...
All diabetes rates are obtained from the Center of Disease Control and Prevention (CDC) Diabetes Survellence Survey site. The data is age-adjusted to only people at least 18 years old. 
...
fitpd['ID_r'] = np.select([fitpd['ID'].isin(ne),fitpd['ID'].isin(south),fitpd['ID'].isin(west),fitpd['ID'].isin(mw)],[1,2,3,4],default = np.nan)
